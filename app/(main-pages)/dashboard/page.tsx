        ...
        {stats.map((stat) => (
          <div key={stat.name} className="relative overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 p-5 sm:p-6 shadow-sm ring-1 ring-slate-200/50 dark:ring-slate-800 transition-all hover:shadow-md hover:-translate-y-1 group">
            ...
            <div className="flex flex-row items-center justify-between space-y-0 pb-3">
              ...
              <div className={`h-2.5 w-2.5 rounded-full shadow-sm shrink-0 ${stat.status === 'urgent' ? 'bg-red-500 animate-pulse shadow-red-500/50' : stat.status === 'warning' ? 'bg-amber-500 shadow-amber-500/50' : 'bg-emerald-500 shadow-emerald-500/50'}`} />
            </div>
            <div>
              <div className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white tracking-tight">{stat.value}</div>
        ...
        <div className="lg:col-span-3">
          <div className="relative rounded-2xl border bg-white dark:bg-slate-900 shadow-sm ring-1 ring-slate-200/50 dark:ring-slate-800 h-full flex flex-col">
            <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Current hardware allocation</p>
            </div>
                ...
                {assetData.map((item, i) => {
                  ...
                  return (
                    <div key={item.name} className="flex items-center gap-2">
                       <span className={`w-3 h-3 rounded-full shrink-0 ${colors[i % colors.length]}`}></span>
                       <span className="text-sm text-slate-600 dark:text-slate-300 font-medium truncate">{item.name}</span>
        ...
        <div className="lg:col-span-4">
          <div className="relative overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 shadow-sm ring-1 ring-slate-200/50 dark:ring-slate-800 h-full flex flex-col">
            <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0">
              ...
              <button className="text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 font-medium self-start sm:self-auto bg-red-50 dark:bg-red-500/10 px-3 py-1.5 rounded-lg transition-colors">View All</button>
            </div>
            <div className="p-0 flex-1 overflow-x-auto">
              <div className="divide-y divide-slate-100 dark:divide-slate-800 min-w-[280px]">
                {[
                  { id: 'T-1029', title: 'Broken monitor in Room 101', user: 'j.smith@company.com', status: 'Open', time: '2h ago' },
                  { id: 'T-1028', title: 'VPN connection failing', user: 'a.jones@company.com', status: 'In Progress', time: '4h ago' },
                  { id: 'T-1027', title: 'Need access to Marketing drive', user: 'm.doe@company.com', status: 'Resolved', time: '1d ago' },
                  { id: 'T-1026', title: 'Laptop battery draining fast', user: 's.taylor@company.com', status: 'Open', time: '1d ago' },
                ].map((ticket) => (
                  <div key={ticket.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 p-4 sm:p-5 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{ticket.user}</p>
                      </div>
                    </div>
                    <div className="flex items-center sm:flex-col justify-between sm:justify-center gap-2 sm:gap-1 pl-13 sm:pl-0 pt-2 sm:pt-0 border-t sm:border-0 border-slate-50 dark:border-slate-800/50">
                      <div className={`text-[11px] font-semibold px-2.5 py-0.5 sm:py-1 rounded-full whitespace-nowrap 
                        ${ticket.status === 'Open' ? 'bg-amber-100/80 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' : 
                          ticket.status === 'In Progress' ? 'bg-blue-100/80 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400' : 
                          'bg-emerald-100/80 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400'}`}>
                        {ticket.status}
                      </div>
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{ticket.time}</span>
                    </div>
                  </div>
                ))}
              </div>
