...
export async function createTicket(data: {
  ...
  category?: string; assetId?: number; createdById: number
}) {
  const [ticket] = await db.insert(tickets).values(data).returning()
  // Log creation event
  await db.insert(ticketEvents).values({
    ticketId: ticket.id, actorId: data.createdById,
    eventType: 'created', newValue: 'open',
  })
  return ticket
}

export async function updateTicket(id: number, actorId: number, data: Partial<{
  title: string; description: string; status: 'open' | 'in_progress' | 'resolved' | 'closed'
  priority: string; assignedToId: number
}>) {
  const existing = await getTicketById(id)
  if (!existing) return null

  const [ticket] = await db.update(tickets).set({ ...data, updatedAt: new Date() } as Parameters<ReturnType<typeof db.update>['set']>[0])
  ...
  if (data.status && data.status !== existing.status) {
    await db.insert(ticketEvents).values({
      ...
      oldValue: existing.status, newValue: data.status,
    })
  }
